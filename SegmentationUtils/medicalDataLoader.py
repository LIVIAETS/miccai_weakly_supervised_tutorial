...
def make_dataset(root, mode):
    ...
    # Testing dataset
        ...
        train_mask_path = os.path.join(root, 'test', 'Img')
        ...
        for it_im, it_gt in zip(images, labels):
            item = (os.path.join(train_img_path, it_im),os.path.join(train_img_path, it_im))
            items.append(item)
...
class MedicalImageDataset(Dataset):
    ...
    def __init__(self, mode, root_dir, transform=None, mask_transform=None, augment=False, equalize=False):
        """
        Args:
            mode (string): Specify which data to load: train/val/test.
            root_dir (string): Directory with all the images.
            transform (callable, optional): Optional transform to be applied on a sample.
            mask_transform (callable, optional): Optional transform to be applied on a sample mask (same as to the image).
            augment (callable, optional): Optional data augmentation to be applied on a sample mask (same as to the image).
        """
